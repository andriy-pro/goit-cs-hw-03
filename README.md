...
Проєкт розділено на логічні модулі для кращої підтримки та масштабованості:

- **`config.py`**:  
  - Зберігає конфігураційні параметри для підключення до MongoDB (URI, назва бази та колекції).
  - Містить налаштування логування (створення окремого лог-файлу з часовою міткою).
  - Містить кольорові схеми для покращення візуального виводу та шаблони повідомлень.
  - Має функцію `validate_config()` для перевірки коректності `MONGO_URI`, `DATABASE_NAME` та `COLLECTION_NAME`.

- **`db_connection.py`**:  
  - Відповідає за підключення до бази даних MongoDB за допомогою `MongoClient`.
  - Виконує перевірку з’єднання (`server_info()`) та логування статусу.
  - Повертає об’єкт колекції для подальшої роботи.

- **`cats_manager.py`**:  
  - Містить CRUD-функції для роботи з документами. 
  - Кожна функція логує успішні дії та помилки, використовує кольоровий вивід для зручності в терміналі.

- **`main.py`**:  
  - Здійснює взаємодію з користувачем через текстове меню.
  - Пропонує користувачу обрати операцію (CRUD) та виконує її.
  - Обробляє помилки вводу користувача та логує неочікувані ситуації.
...
2. **Перевірка роботи контейнера:**
   ...
   Якщо MongoDB працює, контейнер `mongodb` буде у списку.

3. **Переконатися, що порт відкритий:**
